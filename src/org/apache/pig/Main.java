...
static int run(String args[], PigProgressNotificationListener listener) {
    ...
    try {
        ...
        //AVK - Start
        String commandLine = "";
        if(execType == ExecType.STRATO){
        	commandLine = SLoadFunc.join((AbstractList<String>)Arrays.asList(args), " ");
        	// set default stratosphere-related properties
        	final String PIGCONTEXTFILE = ".pig_context";
        	final String PIGPACTPLAN = ".pig_pactplan";
        	properties.put("pig.context.filepath", USER_HOME + File.separator  + PIGCONTEXTFILE);
        	properties.put("pig.pactplan.filepath", USER_HOME + File.separator  + PIGPACTPLAN);
        }
        else{
        	commandLine = LoadFunc.join((AbstractList<String>)Arrays.asList(args), " ");
        }
        //AVK -End
        ...
        case FILE: {
            ...
            if (embedded) {
            	System.out.println("embedded=true");
...
private static String getVersionString() {
	String findContainingJar = JarManager.findContainingJar(Main.class);
	  try { 
		  //AVK: this path needs to point to pig home folder
		  if(findContainingJar==null) findContainingJar = USER_HOME + File.separator + "pig.jar";	
