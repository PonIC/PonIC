...
public class CompilerUtils {
    ...
    //copy of method above
	public static void addEmptyBagOuterJoin(PactPlan fePlan, Schema inputSchema) throws PlanException {
		org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.POProject relationProject = (org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.POProject) fePlan.getRoots().get(0);
	        try {
	            
	            // condition of the bincond
	        	org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.POProject relationProjectForIsEmpty = relationProject.clone();
	            fePlan.add(relationProjectForIsEmpty);
	            String scope = relationProject.getOperatorKey().scope;
	            FuncSpec isEmptySpec = new FuncSpec(IsEmpty.class.getName());
	            Object f = PigContext.instantiateFuncFromSpec(isEmptySpec);
	            org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.POUserFunc isEmpty = new org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.POUserFunc(new OperatorKey(scope, NodeIdGenerator.getGenerator().
	                        getNextNodeId(scope)), -1, null, isEmptySpec, (EvalFunc) f);
	            isEmpty.setResultType(DataType.BOOLEAN);
	            fePlan.add(isEmpty);
	            fePlan.connect(relationProjectForIsEmpty, isEmpty);
	            ...
	            // lhs of bincond (const bag with null fields)
	            org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.ConstantExpression ce = new org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.ConstantExpression(new OperatorKey(scope,
	                    NodeIdGenerator.getGenerator().getNextNodeId(scope)));
	            // the following should give a tuple with the
	            // required number of nulls
	            Tuple t = TupleFactory.getInstance().newTuple(inputSchema.size());
	            ...
	            //rhs of bincond is the original project
	            // let's set up the bincond now
	            org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.POBinCond bincond = new org.apache.pig.backend.stratosphere.executionengine.pactLayer.expressionOperators.POBinCond(new OperatorKey(scope,
	                    NodeIdGenerator.getGenerator().getNextNodeId(scope)));
	            bincond.setCond(isEmpty);
	            bincond.setLhs(ce);
	            bincond.setRhs(relationProject);
